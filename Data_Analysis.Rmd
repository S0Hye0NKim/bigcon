...
glm_SK_Age_Binom <- glm(Non_zero ~ ., data = SK_Age_HDONG_Modified %>% select(-MONTH, -STD_YM, -STD_YMD, -HDONG_NM, -Avg_pop), family = binomial(link = "logit"))
...
```{r}
summary(glm_SK_Age_Binom_)
```


```{r}
summary(glm_SK_Age_Gamma_)
```
...
```{r warning = FALSE}
gam_Dist <- gam(Real_Value ~ Month + ADMD_CD + Category + s(avg_temp) + s(avg_pm10)  + s(avg_pm25) + pm10_CTG + pm25_CTG + pm10_Wrn + pm25_Wrn + ADMD_CD:Category + ADMD_CD:Month + Month:Category, data = Real_Dist)
```
