...
tibble(HDONG_CD = rep(HDong_CD$HDONG_CD, each = 6), 
  ...
  mutate(Estimate = Estimate_Dust + Estimate_HDONG + Estimate) %>%
  select(HDONG_CD, pm25_CTG, Estimate) %>%
...
tibble(HDONG_CD = rep(HDong_CD$HDONG_CD, each = 6), 
  ...
  left_join(SK_Age_inter_HDONG, by = c("HDONG_CD", "pm25_CTG")) %>%
  mutate_all(replace_na, replace = 0) %>%
  mutate(Estimate = Estimate + Estimate_Dust) %>%
...
tibble(Sex = rep(c("MAN", "WMAN"), each = 6), 
  ...
  mutate(Estimate = Estimate %>% replace_na(0), 
         Estimate_Sex = rep(c(0, summary_SK_Age %>% filter(Variable == "SexWMAN") %>% 
                                .$Estimate), each = 6), 
         Estimate = Estimate + Estimate_Sex) %>%
  ggplot() +
...
tibble(AGE_CTG = rep(c("Youth", "Rising", "Middle", "Senior"), 6), 
  ...
  mutate_all(replace_na, replace = 0) %>%
  mutate(Estimate = Estimate_Inter + Estimate_Dust ) %>%
  ggplot() +
...
SK_Time_coef <- tibble(Week_Day = rep(Week_Day, each = 12), 
...
SK_Time_coef %>%
  mutate(Estimate = val_Week + val_Dust + val_Hol + val_Week_Hol + val_Week_pm25 + val_pm25_Hol + val_inter) %>%
