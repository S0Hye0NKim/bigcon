...
SK_Age_Main_Eff_HDONG <- summary_SK_Age %>%
  filter(p_val <= 0.05, 
         Variable %>% str_starts("HDONG_CD"), 
         Variable %>% str_ends(DGT)) %>%
  mutate(HDONG_CD = parse_number(Variable) %>% as.character, 
         pm25_CTG = "Good") %>%
  select(HDONG_CD, pm25_CTG, Estimate)

SK_Age_inter_HDONG <- summary_SK_Age %>%
  filter(p_val <= 0.05) %>%
...
tibble(HDONG_CD = rep(HDong_CD$HDONG_CD, each = 6), 
  ...
  left_join(SK_Age_Main_Eff_Dust %>% select(-HDONG_CD), by = "pm25_CTG") %>%
  left_join(SK_Age_Main_Eff_HDONG %>% select(-pm25_CTG), by = "HDONG_CD", suffix = c("_Dust", "_HDONG")) %>%
  left_join(SK_Age_inter_HDONG, by = c("HDONG_CD", "pm25_CTG")) %>%
  ...
  mutate(Correction = case_when(pm25_CTG == "Good" ~ 10, 
                                ...
                                pm25_CTG == "Worst" ~ 115), 
         Estimate = Estimate_Dust + Estimate_HDONG + Estimate + SK_Age_pm25_coef * Correction) %>%
  select(HDONG_CD, pm25_CTG, Estimate) %>%
...
tibble(HDONG_CD = rep(HDong_CD$HDONG_CD, each = 6), 
  ...
  mutate(Correction = case_when(pm25_CTG == "Good" ~ 10, 
                                ...
                                pm25_CTG == "Very Unhealthy" ~ 60, 
                                pm25_CTG == "Worst" ~ 115), 
         Estimate = Estimate + Estimate_Dust + SK_Age_pm25_coef * Correction) %>%
  filter(pm25_CTG != "Good") %>%
  select(-Estimate_Dust, -Estimate_HDONG) %>%
  left_join(HDONG_map, by = "HDONG_CD") %>%
  ggplot() +
  geom_polygon(aes(x = long, y = lat, group = group, fill = Estimate), color = "black") +
  scale_fill_gradient2(low = "red", high = "blue") +
...
Correction 값으로는 pm25_CTG 별 avg_pm25의 평균 값 사용
...
tibble(Sex = rep(c("MAN", "WMAN"), each = 6), 
  ...
  mutate(Estimate = Estimate %>% replace_na(0), 
         Estimate_Sex = rep(c(0, summary_SK_Age %>% filter(Variable == "SexWMAN") %>% 
                                .$Estimate), each = 6),
         Correction = case_when(pm25_CTG == "Good" ~ 10, 
                                ...
                                pm25_CTG == "Very Unhealthy" ~ 60, 
                                pm25_CTG == "Worst" ~ 115), 
         Estimate = Estimate + Estimate_Sex + SK_Age_pm25_coef * Correction) %>%
  ggplot() +
...
tibble(AGE_CTG = rep(c("Youth", "Rising", "Middle", "Senior"), 6), 
  ...
  left_join(summary_SK_Age %>%
              ...
              mutate(AGE_CTG = AGE_CTG %>% str_remove("AGE_CTG"), 
                     pm25_CTG = pm25_CTG %>% str_remove("pm25_CTG")) %>%
              select(AGE_CTG, pm25_CTG, Estimate), by = c("AGE_CTG", "pm25_CTG")) %>%
  left_join(SK_Age_Main_Eff_Dust %>% select(-HDONG_CD), by = "pm25_CTG", suffix = c("_Inter", "_Dust")) %>%
  ...
  mutate(Correction = case_when(pm25_CTG == "Good" ~ 10, 
                                ...
                                pm25_CTG == "Very Unhealthy" ~ 60, 
                                pm25_CTG == "Worst" ~ 115), 
         Estimate = Estimate_Inter + Estimate_Dust+ SK_Age_pm25_coef * Correction) %>%
  ggplot() +
...
```{r}
Target_HDONG <- c("11350619", "11350600", "11110640", "11110680", "11110550")
People %>%
  ...
  mutate(Type = ifelse(HDONG_CD == 11110550, "-", "+"))%>%
...
```{r}
glm_Dist_Best <- glm(AMT_IND ~ Month + avg_temp + ADMD_CD + pm10_CTG + 
    pm25_CTG + Week_Day + pm10_Wrn + pm25_Wrn + Holiday + pm10_CTG:Holiday + 
    pm25_CTG:Holiday, data = Dist_Modified %>% na.omit(), family = Gamma(link = "log"))
