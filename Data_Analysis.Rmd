...
```{r}
glm_SK_Age_Gamma_Hol_HDCTG <- glm(Avg_pop ~ HDONG_CD + Sex + Age + AGE_CTG + avg_temp + avg_pm10 + avg_pm25 + pm10_CTG + pm25_CTG + pm25_Wrn + Week_Day + Holiday + Sex:AGE_CTG + Holiday:Sex + Holiday:AGE_CTG + Holiday:pm10_CTG + HDONG_CD:pm25_CTG, 
                         data = SK_Age_HDONG_Modified %>% filter(Non_zero == 1) %>%
                          select(-MONTH, -STD_YM, -STD_YMD, -HDONG_NM, -Non_zero), 
                         family = Gamma(link = "log"))
```
...
```{r}
SK_Age_inter <- summary_SK_Age %>%
  filter(p_val <= 0.05) %>%
  filter(Variable %>% str_starts("HDONG_CD" %R% one_or_more(DGT) %R% ":" %R% "pm25_")) %>%
  separate(Variable, into = c("HDONG_CD", "pm25_CTG"), sep = ":") %>%
  mutate(HDONG_CD = parse_number(HDONG_CD) %>% as.character, 
         pm25_CTG = pm25_CTG %>% str_remove("pm25_CTG"))

SK_Age_Main_Eff <- summary_SK_Age %>%
  filter(p_val <= 0.05) %>%
  filter(Variable %>% str_starts("pm25_CTG")) %>%
  mutate(HDONG_CD = "11110515", 
         pm25_CTG = Variable %>% str_remove("pm25_CTG")) %>%
  select(HDONG_CD, pm25_CTG, Estimate, Std_Er, t_val, p_val)
```

```{r}
tibble(HDONG_CD = rep(HDong_CD$HDONG_CD, each = 6), 
       pm25_CTG = rep(Dust_level, 36)) %>%
  left_join(bind_rows(SK_Age_Main_Eff, SK_Age_inter) %>%
  select(HDONG_CD, pm25_CTG, Estimate), by = c("HDONG_CD", "pm25_CTG")) %>%
  mutate(Estimate = Estimate %>% replace_na(replace = 0)) %>%
...
```{r eval = FALSE}
glm_SK_Time_Binom <- glm(Non_zero ~ ., data = SK_Time_HDONG_Modified_ %>% select(-STD_YM, -STD_YMD, -HDONG_NM, -Avg_pop), family = binomial)
...
```{r}
glm_SK_Time_Gamma_Hol_HD_CTG <- glm(Avg_pop ~ HDONG_CD + Time + Week_Day + avg_temp + avg_pm10 + avg_pm25 + pm10_CTG + pm25_CTG + pm10_Wrn + pm25_Wrn + Holiday + Time_Group + Holiday:Time_Group + Holiday:pm10_CTG + Holiday:pm25_CTG + Holiday:pm10_Wrn + Holiday:pm25_Wrn + HDONG_CD:pm25_Wrn, 
                          data = SK_Time_HDONG_Modified_ %>% filter(Non_zero == 1), 
                          family = Gamma(link = "log"))
```
...
glm_Dist_All <- glm( AMT_IND ~ Month + avg_temp + avg_pm10 + avg_pm25 +
                     factor(HDONG_NM) + pm10_CTG + pm25_CTG + pm10_Wrn + pm25_Wrn + Holiday + pm10_CTG:Holiday + pm25_CTG:Holiday + pm10_Wrn:Holiday + pm25_Wrn:Holiday, 
...
```{r}
glm_Dist_Best <- glm(AMT_IND ~ Month + avg_temp + ADMD_CD + pm10_CTG + pm25_CTG + 
    pm10_Wrn + pm25_Wrn + Holiday + pm10_CTG:Holiday + pm25_CTG:Holiday, data = Dist_Modified %>% na.omit(), family = Gamma(link = "log"))
...
```{r}
gam_Dist <- mgcv::gam(Real_Value ~ Month + ADMD_CD + Category + s(avg_temp) + s(avg_pm25) + s(avg_pm10) + pm10_CTG + pm25_CTG + pm10_Wrn + pm25_Wrn + ADMD_CD:Category + ADMD_CD:Month + Month:Category + Holiday +  Category:pm10_CTG + Category:pm25_CTG, data = Real_Dist, select = TRUE)
```
...
preds_gam_Dist1 <- predict.gam(gam_Dist, newdata = Real_Dist %>% na.omit, type = "response")
```
...
```{r}
glm_NB_Card <- MASS::glm.nb(USE_CNT ~ ., data = Card_Daily %>% select(-STD_DD, -AGE_CD, -USE_AMT, -pm10_Wrn, -avg_pm10))
